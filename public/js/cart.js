...
app.showCart = function () {
	...
	if (list.length === 0) {
		...
		for (let i = 0; i < list.length; i++) {
			let data = list[i];
			let item = app.createElement("div", {
				atrs: {
					className: "row"
				}
			}, container);
			// variant
			let variant = app.createElement("div", {
				atrs: {
					className: "variant"
				}
			}, item);
			let picture = app.createElement("div", {
				atrs: {
					className: "picture"
				}
			}, variant);
			app.createElement("div", {
				atrs: {
					className: "details",
					innerHTML: data.title + "<br/>" + data.id + "<br/><br/>顏色：" + data.color.name + "<br/>尺寸：" + data.size
				}
			}, variant);
			...
			// qty
			let qty = app.createElement("div", {
				atrs: {
					className: "qty"
				}
			}, item);
			let qtySelector = app.createElement("select", {
				atrs: {
					index: i
				},
				evts: {
			...
			for (let j = 1; j <= data.stock; j++) {
				qtySelector.add(app.createElement("option", {
					atrs: {
						value: j,
						textContent: j
					}
